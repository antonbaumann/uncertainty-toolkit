"""
Implementation of the MIMO UNet architecture.

Code from https://github.com/antonbaumann/MIMO-Unet
"""
...
from .utils import create_module_list
...
class MimoUNet(nn.Module):
    ...
    def __init__(
        ...
        self.encoder: SubnetworkEncoder = SubnetworkEncoder(
        ...
        self.core: SubnetworkCore = SubnetworkCore(
            num_subnetworks=num_subnetworks,
            filter_base_count=filter_base_count,
        ...
        self.decoder: SubnetworkDecoder = SubnetworkDecoder(
            num_subnetworks=num_subnetworks,
            filter_base_count=filter_base_count,
...
class SubnetworkDecoder(nn.Module):
    ...
    def apply_input_transform(
        ...
        Args:
            image (torch.Tensor): [B, C_image, H, W]
            label (torch.Tensor): [B, C_label, H, W]
            mask (torch.Tensor): [B, 1, H, W]
            num_subnetworks (int): The number of subnetworks.
            input_repetition_probability (float): The input repetition probability.
            batch_repetitions (int): The number of batch repetitions.
